...
const InputBar = ({ type, id, name, value, placeholderMsg, disabled, required }) => {
  // disabled={ true | false }
  ...
  return (
    <Input type={type} id={id} name={name} required={required} value={value} placeholder={`${placeholderMsg}`} style={{ width: '200px' }} size='large' disabled={disabled}/>
  )
};
